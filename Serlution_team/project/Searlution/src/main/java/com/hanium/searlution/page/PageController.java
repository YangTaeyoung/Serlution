...
public class PageController {
	...
	String goGraph(HttpServletRequest request, Model model, @RequestParam("keyword") String keyword) throws UnsupportedEncodingException
	{
		...
		String[] tmpStr = new NaverGraph(keyword, "oneMonth", null, null, null).graphStr();
	...
	// 그래프 옵션 바꿀 시 컨트롤러
	@RequestMapping(value="chart/graph", method=RequestMethod.GET, params = {"keyword", "date", "range", "gender", "age"})
	String goGraph(HttpServletRequest request, Model model, @RequestParam("keyword") String keyword
			, @RequestParam("date") String date, @RequestParam("range") String[] ranges, @RequestParam("gender") String[] genders
			, @RequestParam("age") String[] ages) throws UnsupportedEncodingException
	{
		request.setCharacterEncoding("UTF-8");
		HttpSession session = request.getSession();
		
		String[] tmpStr = new NaverGraph(keyword, date, ranges, genders, ages).graphStr();
		// model.addattribute("변수명: 넘길 값의 이름", 넘길 실제 값): jsp 페이지로 넘기고 싶은 값을 전달함.  
		// 받을 떄는 JSTL을 통해서 받아야 함. JSP 내부에서 사용하는 예시: ${변수명} 밑에 키워드 값을 넘기는 예제 있음.
		model.addAttribute("label", tmpStr[0]);
		model.addAttribute("data", tmpStr[1]);
